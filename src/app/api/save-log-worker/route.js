...
// 1. Your exact email alert function
...
export async function POST(request) {
  try {
    ...
    // The worker sends `deviceMac`, so we look up the patient who owns that device
    ...
    const newLog = await prisma.sensorLog.create({
      data: {
        patientId:   patient.id, // Now using the ID we found from the MAC address
