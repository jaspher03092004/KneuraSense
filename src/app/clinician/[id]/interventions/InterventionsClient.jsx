...
import { 
  ...
  User, CheckCircle, Clock, Filter, ChevronDown, Search, Activity as ActivityIcon
} from 'lucide-react';
...
export default function InterventionsClient({ clinicianId, allPatients, interventions }) {
  ...
  // History Timeline State (Pagination & Filters)
      ...
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 transition-colors duration-300">
              ...
              <form id="intervention-form" action={handleAddNote} className="space-y-4">
                ...
                <div className="relative">
                  ...
                  >
                    <span className={selectedPatient ? "text-slate-900 dark:text-white font-medium" : "text-slate-400"}>
          ...
          <div className="lg:col-span-2 z-10">
            <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 md:p-8 transition-colors duration-300 min-h-full">
              
              {/* Timeline Header & Filters */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <Calendar className="text-slate-400" size={20} /> Intervention History
                </h2>
                
                {/* FILTERS CONTAINER */}
                <div className="flex flex-wrap items-center gap-3">
                  
                  {/* UNIFORM STATUS FILTER */}
                  <div className="relative">
                    <button 
                      onClick={() => setIsStatusDropdownOpen(!isStatusDropdownOpen)}
                      className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-blue-500 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl px-4 py-2.5 outline-none transition-colors"
                    >
                  ...
                  {/* UNIFORM SEARCHABLE PATIENT FILTER */}
                  <div className="relative">
                    <button 
              ...
              <div className="relative border-l-2 border-slate-100 dark:border-slate-800 ml-3 md:ml-4 space-y-8 pb-4">
                {displayedInterventions.length === 0 ? (
                  ...
                  displayedInterventions.map((record) => {
                    ...
                    return (
                      <div key={record.id} className="relative pl-8 md:pl-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-5 border border-slate-100 dark:border-slate-700/50 hover:border-slate-200 dark:hover:border-slate-600 transition-colors">
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1">
                                  <User size={12}/> {record.patient.fullName}
                                </span>
                              </div>
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700/50 flex items-center justify-between">
                            {record.isAcknowledged ? (
                              <span className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1.5 rounded-lg border border-emerald-100 dark:border-emerald-500/20">
                                <CheckCircle size={14} />
                                Patient Acknowledged ({new Date(record.acknowledgedAt).toLocaleDateString()})
                              </span>
                            ) : (
                              <span className="flex items-center gap-1.5 text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-100 dark:border-amber-500/20">
                                <Clock size={14} />
              ...
              {hasMore && (
                <div className="mt-8 flex justify-center">
                  <button 
                    ...
                    className="flex items-center gap-2 px-6 py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-full text-sm font-bold transition-colors"
                  >
